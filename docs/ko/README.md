...
| 2026-06-22 11:29:42 | v1.6.22 - 🔁 타사 직접 호출 '동일 모델 다중 프로바이더' 자동 페일오버: 하나의 모델 ID에 API 관리에서 여러 타사를 구성하면 직접 호출은 첫 번째를 고정 우선하고, 오류 / 속도 제한 / 쿼터 소진 / 타임아웃 / 빈 응답 시 동일 이름의 다음 타사로 자동 전환하며 모두 실패할 때만 마지막 오류 반환(클라이언트는 단일 모델 이름으로 무감각); 스트리밍은 첫 바이트 전에 페일오버, 실패한 프로바이더는 메모리 내 쿨다운(`THIRDPARTY_FAILOVER_COOLDOWN`, 기본 180초)에 들어가 만료까지 우선 건너뛰되 모두 쿨다운 중이거나 하나뿐이면 그래도 시도(고갈 없음); 기본 활성화·스위치 없음, 단일 프로바이더 무회귀, Gemini→타사 폴백 체인에 영향 없음 |
