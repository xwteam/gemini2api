...
| 2026-06-22 11:29:42 | v1.6.22 - 🔁 サードパーティ直接呼び出しの「同一モデル複数プロバイダ」自動フェイルオーバー：1 つのモデル ID に API 管理で複数のサードパーティを設定した場合、直接呼び出しは最初の 1 社を固定優先し、エラー / レート制限 / クォータ枯渇 / タイムアウト / 空レスポンス時に同名の次のサードパーティへ自動切替、全滅時のみ最後のエラーを返す（クライアントは単一モデル名で無感知）；ストリーミングは最初のバイト前にフェイルオーバー、失敗したプロバイダはメモリ内クールダウン（`THIRDPARTY_FAILOVER_COOLDOWN`、既定 180 秒）に入り期限まで優先スキップ、ただし全社クールダウン中または 1 社のみの場合も試行（枯渇させない）；既定で有効・スイッチなし、単一プロバイダは後方互換、Gemini→サードパーティのフォールバックチェーンに影響なし |
| 2026-06-21 00:33:02 | v1.6.21 - 🔀 Gemini → 第三者自動フォールバックチェーン：任意の Gemini モデル（flash/pro/thinking）がエラーまたは空レスポンスを返した場合、API キープール内の第三者モデルに自動で切り替えてネイティブにリトライ、クライアントは無感知で単一モデル名のまま；候補はプールから自動取得し名前で非チャットモデル（image/video/audio/embedding 等）を除外、ランダムにローテーションして失敗（エラー/空）したら次へ、非ストリーミングで検証（エラーも空も成功と誤認しない）・ストリーミングは SSE（ネイティブツール呼び出し含む）に変換；`FALLBACK_ENABLED` 既定無効、`FALLBACK_MODELS` で精密指定可、後方互換 |
