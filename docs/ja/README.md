...
| 日付 | 更新内容 |
|------|----------|
| 2026-05-31 21:00:00 | v1.6.6 - 🖼️ 生成画像のローカルホスティング：チャット API の画像生成結果をアクセス可能なローカル URL（/images/{id}）で返却するように変更し、CLI / エージェント系クライアントでも正常にレンダリング表示できるように（base64 はこれらのクライアントでは表示不可）；画像は定期的に自動クリーンアップ |
